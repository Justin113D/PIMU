...
namespace J113D.Pimu.Desktop.App.scripts.Input.Config
{
	public static class InputMappingConfig
	{
		...
		private const string _configMouseSection = "Mouse";
		private const string _configMouseSensitivityKeyVertical = "Vertical sensitivity";
		private const string _configMouseSensitivityKeyHorizontal = "Horizontal sensitivity";

		private static readonly Dictionary<StringName, InputEvent?> _defaultEvents = [];

		public static Vector2 MouseSensitivity { get; set; }
		...
		public static ConfigFile ToConfigFile()
		{
			...
			result.SetValue(_configMouseSection, _configMouseSensitivityKeyHorizontal, MouseSensitivity.X);
			result.SetValue(_configMouseSection, _configMouseSensitivityKeyVertical, MouseSensitivity.Y);
		...
		public static void Load(ConfigFile config)
		{
			...
			Vector2 sensitivity = default;

			try
			{
				sensitivity.X = (float)config.GetValue(_configMouseSection, _configMouseSensitivityKeyHorizontal);
			}
			catch
			{
				sensitivity.X = 1f;
			...
			try
			{
				sensitivity.Y = (float)config.GetValue(_configMouseSection, _configMouseSensitivityKeyVertical);
			}
			catch
			{
				sensitivity.Y = 1f;
